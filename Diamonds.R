# read in data from: http://jse.amstat.org/v9n2/4Cdata.txt
...
1.02 H VS2  HRD  8959
1.09 I VVS2 HRD  9107", sep = "", header = FALSE)
...
#explore data to find best for fitting model
plot(diamonds$Carat, diamonds$Price)
plot(log(diamonds$Carat), log(diamonds$Price))
#the ln of the data appears to better fulfill assumptions of:
# ADDITIVE relationship, CONSTANT VARIANCE, and FEW (to no) OUTLIERS

#Analysis:
#response variable, explanatory variable (insert here)

#model: log(Price) = beta0 + beta1 log(Carat) + epsilon, epsilon~N(0,sigma^2)
diamonds_out <- lm(log(Price) ~ log(Carat), data = diamonds)

#table of estimates and std errors and t-statistic
summary(diamonds_out)

#interpretation of log(Carat) estimate from table:
#For a one unit increase in log(Carat), we estimate 
# an increase in log(Price) of 1.537 on average
...
#create prediction graph with bands
#attach library
library(ggplot2)

#create plot that proves strength of model to jewelry store owner
qplot(log(Carat), log(Price), data = diamonds, geom = 'smooth', formula = y ~ x,
      method = 'lm', se = TRUE, xlab = 'Diamond Size', ylab = 'Diamond Price Estimate')

#create plot for couple that is ring shopping
#remember to untransform using exp at the beginning (as below)
exp(predict(diamonds_out, newdata=data.frame(Carat = 1), interval='prediction'))

#create prediction graphic
plot_df <- cbind(diamonds, exp(predict(diamonds_out, interval = "prediction")))
ggplot(plot_df, aes(Carat, Price)) +
    xlab('Diamond Carats') +
    ylab('Diamond Price') +
    geom_point() +
...
#we can't really use r-squared (read here for info http://blog.minitab.com/blog/adventures-in-statistics-2/regression-analysis-how-do-i-interpret-r-squared-and-assess-the-goodness-of-fit)
# because r-squared is taking the log(carat) and log(price) rather than untransformed values

#calculate better goodness-of-fit measure
